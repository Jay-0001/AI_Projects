...
def winning_move(board, piece):
	...
	for c in range(COLUMN_COUNT-3):
		for r in range(3, ROW_COUNT):
			if board[r][c] == piece and board[r-1][c+1] == piece and board[r-2][c+2] == piece and board[r-3][c+3] == piece:
				return True
#can scaled upto connect -n by modifying the above function

def evaluate_window(window, piece):
#window ~ helper for score calcn
	score = 0
...
def minmax(board, depth, alpha, beta, maximizingPlayer):
	#check for legally possible moves before making one
	...
	if maximizingPlayer:
		...
		for col in valid_locations:
			...
			b_copy = board.copy()                       #took a while
...
def pick_best_move(board, piece):
    #checking for the best move by mamking all possible moves at an instant
	valid_locations = get_valid_locations(board)
	best_score = -10000
